...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $pdo->prepare("INSERT INTO rooms (room_number, room_type_id, floor, status) VALUES (?, ?, ?, ?)");
        $stmt->execute([$room_number, $room_type_id, $floor, $status]);
        header('Location: /pms_hotel/rooms/?msg=created');
        exit;
    }
}
...
<main class="main-content">
    ...
    <?php if (count($roomTypes) == 0): ?>
        <div class="alert alert-danger">
            ...
            No room types found. Please <a href="/pms_hotel/rooms/types.php">create room types</a> first.
        </div>
    <?php endif; ?>
